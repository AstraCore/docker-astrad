...
ASTRA_IMAGE=${ASTRA_IMAGE:-Astra_Linux/astrad}
...
docker kill astrad-node 2>/dev/null || true
docker rm astrad-node 2>/dev/null || true
stop docker-astrad 2>/dev/null || true

# Always pull remote images to avoid caching issues
if [ -z "${ASTRA_IMAGE##*/*}" ]; then
    docker pull $ASTRA_IMAGE
fi

# Initialize the data container
docker volume create --name=astrad-data
docker run -v astrad-data:/astra --rm $ASTRA_IMAGE astra_init

# Start astrad via upstart and docker
curl https://raw.githubusercontent.com/AstraCore/docker-astrad/master/upstart.init > /etc/init/docker-astrad.conf
start docker-astrad

set +ex
echo "Resulting astra.conf:"
docker run -v astrad-data:/astra --rm $ASTRA_IMAGE cat /astra/.astra/astra.conf
